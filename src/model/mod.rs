...
impl Model {
    ...
    /// # Errors
    ///
    /// Returns an error if CSV processing fails or model initialization encounters issues.
    pub fn analyse_file(
        &self,
    ...
    fn run_sigmoid_inference_batched(
        ...
        let num_batches = encodings.len().div_ceil(MAX_BATCH_SIZE);
...
impl InitializedModel {
    ...
    /// # Errors
    ///
    /// Returns an error if the configuration file cannot be loaded or model initialization fails.
    ///
    /// # Example
    /// ```no_run
    ...
    /// # Errors
    ///
    /// Returns an error if CSV processing or model inference fails.
    pub fn analyse_file(
        &mut self,
        csv_file_struct: &CsvFile,
    ) -> Result<(Vec<Anomaly>, u32, u32), Box<dyn Error>> {
